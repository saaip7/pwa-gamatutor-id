...
def init_scheduler(app):
    ...
    with app.app_context():
        ...
        scheduler.add_job(
            job_cleanup_orphan_sessions, "interval", minutes=9, # [FLAG STUDY] test: 9min, prod: hours=6
            id="orphan_cleanup", replace_existing=True,
        )
        scheduler.add_job(
            job_check_idle_sessions, "interval", minutes=2,  # [FLAG STUDY] test: 2min, prod: minutes=10
            id="check_idle_sessions", replace_existing=True,
        )
        scheduler.add_job(
            job_auto_end_stale_sessions, "interval", minutes=2, # [FLAG STUDY] test: 2min, prod: minutes=10
