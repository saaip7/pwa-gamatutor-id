...
const DUMMY_PREFERENCES = {
  character: {
    ...
    equipped: {
      head: "base" as SlotLevel,
      top: "base" as SlotLevel,
      bottom: "base" as SlotLevel,
      special: null as SlotLevel | null,
    },
  },
};

const DUMMY_BADGES = [
  { type: "streak_starter", unlocked: true },
  { type: "focus_master", unlocked: false },
  { type: "reflector", unlocked: false },
  { type: "completer_10", unlocked: false },
  { type: "explorer", unlocked: false },
];

const DUMMY_UNLOCKED_BADGE_TYPES = DUMMY_BADGES.filter((b) => b.unlocked).map((b) => b.type);

// ─── Joyride Steps ──────────────────────────────────────────────

const steps = [
  {
    target: "#guide-wardrobe-character",
    content: "Ini Wardrobe! Setiap milestone belajar yang kamu capai membuka aksesoris baru untuk mempercantik karaktermu.",
    disableBeacon: true,
  },
  {
    target: "#guide-wardrobe-tabs",
    content: "Pilih kategori aksesoris: Kepala, Baju, Kaki, atau Spesial.",
    disableBeacon: true,
  },
  {
    target: "#guide-wardrobe-items",
    content: "Sentuh item untuk memasangnya. Item yang masih terkunci akan terbuka otomatis saat kamu meraih badge tertentu!",
    disableBeacon: true,
  },
];
...
export default function GuideStep4Page() {
  ...
  const [gender, setGender] = useState<Gender>(DUMMY_PREFERENCES.character.gender);
  const [equipped, setEquipped] = useState<Equipped>(DUMMY_PREFERENCES.character.equipped);

  const unlockedBadgeTypes = DUMMY_UNLOCKED_BADGE_TYPES;
  ...
  const handleJoyrideCallback = (data: { status: string }) => {
    if (data.status === STATUS.FINISHED) {
      router.push("/onboarding/guide/complete");
    }
  };
    ...
    <div className="w-full h-screen bg-white flex flex-col mx-auto overflow-hidden relative max-w-md">
      ...
      <div className="shrink-0 pt-3 px-5 sticky top-0 bg-white z-40">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider">Langkah 5 dari 5</span>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto no-scrollbar relative bg-white">
        ...
        {/* Item Grid */}
        <div id="guide-wardrobe-items" className="px-5 py-6 bg-neutral-50 pb-8 flex-1">
